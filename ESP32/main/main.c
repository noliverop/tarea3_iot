...
int save_nvs_config(Config* config){

    int err = nvs_set_str(my_handle, "config", (char*) config);
    if (err != ESP_OK){
        ESP_LOGI(GATTS_TAG, "Failed to save NVS");
    }
...
void save_value(int16_t attr_handle, uint16_t length, const uint8_t *value){
    ESP_LOGI(GATTS_TAG, "Trying to change char value, handle = %d", attr_handle);
    ESP_LOGI(GATTS_TAG, "Config handle = %d", gl_profile_tab[PROFILE_A_APP_ID].char_handle);
    esp_err_t err = esp_ble_gatts_set_attr_value(42, length, value);
    if(err != ESP_OK){
        ESP_LOGE(GATTS_TAG,"Failed to write characteristic"); 
    }
    
    if (attr_handle == gl_profile_tab[PROFILE_A_APP_ID].char_handle){
        Config config; 
        memcpy(&config, value, length);
        save_nvs_config(&config);
        load_sensors_to_char(config.status, config.protocol);
    }
}
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        if (char_len > 20){
            int len = 19; 
            if (param->read.offset + len > char_len){
                len = char_len - param->read.offset;
            }
        }
        rsp.attr_value.len = len;
        for (int i=0; i<len; i++){
            rsp.attr_value.value[i] = char_val[i+param->read.offset]; 
        }
